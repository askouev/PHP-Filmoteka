	...
	<div class="container user-content pt-35">
		...
		<div class="admin-nav mb-20">
			...
			?>
				...
				<a href="login.php" class="admin-nav__link">Вход админа</a>
			<?php
			}
			?>
			...
			if( isAdmin() ){
			?>
				<a href="logout.php" class="admin-nav__link">Выход админа</a> 
			<?php
			}
			?>
		...
		<?php  if( isset($_COOKIE['user-name']) ){  ?>
			...
			<div class="mb-30" >
				...
				<?php  if( isset($_COOKIE['user-city']) ){  ?>
					<h2>Привет, <?=$_COOKIE['user-name']?> из <?=$_COOKIE['user-city']?> !</h2>
				<?php } else { ?>
					<h2>Привет, <?=$_COOKIE['user-name']?> !</h2>
				<?php } ?>
			</div>
